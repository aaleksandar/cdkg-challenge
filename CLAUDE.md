...
**Code runs from the image; data lives in the clone.** In production the ingest container clones `GITHUB_REPO` into `/repo` once, at first boot, and never pulls it on its own; publishing (below) is what brings `main` into it. `KUZU_DIR`, `TRANSCRIPTS_DIR`, `DATA_DIR` and `ENTITIES_JSON` point into that clone because it is where ingestion output accumulates. `PIPELINE_SCRIPTS_DIR` points at `/app`, the image, because that is the code the deploy just built — running `02_domain_graph.py` from the clone meant a fix to `src/kuzu` never reached production. `graph._script_env` names every data path explicitly so the scripts read the clone regardless of where they run from. A changed `GITHUB_REPO` only moves the clone's remote at boot (`docker-entrypoint.sh`); with publishing off, bringing the clone's contents up to date is a deliberate `git` step on the server, because the working copy holds unpublished ingestion output.

**Publishing: a commit is a talk, and the clone lives on the ingest branch.** `GIT_PUSH_ENABLED` is on in `config/deploy.yml`; it is a deploy-time setting, never a panel click, because it writes outside the machine. The publish stage (`gitops.publish_ingest`) commits exactly what the hand-made ingestion commits carried — the `.srt`, `Transcripts/.ingest/<video_id>.json`, the metadata CSV and **`src/kuzu/entities.json`** (the tags; without them a clone of the repository re-spends the LLM call or builds the talk untagged — the first version of the stage omitted this file), plus the HeySummit catalogue when it is dirty — as `Ingest transcript: <talk_title>` with speaker, event and video in the body, on the one long-lived branch `ingest/auto` behind the one pull request "Automated transcript ingestion". Rows the HeySummit sync seeded or a curator edited since the previous talk ride along in the CSV, as they did in the hand-made history; nothing else gets a commit of its own. The order is commit → merge GitHub's ingest branch → push → open or update the PR → merge `main` → push, so the talk is on GitHub before anything that can conflict is attempted, and merging `main` into the branch is how the merged PR and every curation made on GitHub reach the server's graph. The first version committed on the clone's local `main` and force-pushed a copy, which meant that after one publish any other merge into `main` made every later one fail "diverged", recoverable only with `git` on the server. **Two appends to the CSV are merged by TalkID, and to `entities.json` by filename** (`gitops._resolve_by_key`): git sees two appends at the end of a file as a conflict, the data does not, and GitHub's version of a shared entry wins because a curator's edit there was a decision. Any other conflict aborts the merge with the files named; the talk's commit is kept, the PR shows the conflict where GitHub offers to resolve it, and the next publish takes the resolution from the remote branch. A failed publish fails the run — a talk that is not on GitHub is not durable — and the drawer says the graph has it and that "Run the pipeline again" retries only the publish, every earlier stage being content-addressed. That re-run is also how a talk ingested while publishing was off reaches GitHub; there is deliberately no "publish everything" button. At boot, `gitops.adopt_published_history` moves a clean clone onto `ingest/auto` (a rebuilt server's fresh clone is at `main`; the talks published before it are on the branch), never over uncommitted work and never fatally. Authentication is the GitHub App: `GITHUB_APP_ID` and `GITHUB_APP_PRIVATE_KEY` (the PEM, verbatim or base64 on one line — `config._private_key` accepts both, because the deploy writes secrets one value per line) come from the Actions secrets `CDKG_GITHUB_APP_ID` / `CDKG_GITHUB_APP_PRIVATE_KEY` (GitHub forbids secret names starting with `GITHUB_`), scoped to Contents and Pull requests on this repository only, with `main` protected so the App can never push there. Empty, the deploy still goes through, Advanced → "Publish to GitHub" says publishing cannot work, and every run fails at its last stage. `gitops.health()` is configuration only — no network on render. `tests/test_gitops.py` runs the whole flow against a bare repository standing in for GitHub, and `tests/integration/test_publish.py` does it after a real ingestion.
